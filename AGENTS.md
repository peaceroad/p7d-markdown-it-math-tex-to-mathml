...
- Browser: `script/math-tex-to-mathml.js` is generated by `tools/generate-browser-entry.js` from the same MathJax source list (excluding `bussproofs` and `bboldx` from preload).
...
- `mathmlReport` enables per-render findings in `env.mathmlReport` for the selected MathML mode.
  - Findings can include MathJax `<merror>` output (`math-error`), non-Core element names, legacy `mathvariant` usage except `mathvariant="normal"`, browser-mode fallback markers (`style-fallback-mathvariant`, `unsupported-mathvariant`, `unconverted-mathvariant`), and `nested-mathsize` for nested relative font-size declarations.
...
- `tools/generate-browser-entry.js` is the source of truth for the browser entry's static TeX extension imports.
- `tools/generate-math-styles.js` builds the standalone CSS bundles in `style/` from `style-src/`.
- `tools/convert-newcm-woff2.py` is an optional local helper for converting a user-supplied `NewCMMath-Regular.otf` into `NewCMMathCustom-Regular.woff2` with repo-specific webfont metadata.
  It does not download fonts and the repo intentionally does not commit NewCM font assets.
  The npm wrapper assumes `python` is on `PATH`; use `python3 tools/convert-newcm-woff2.py ...` directly on environments that expose Python 3 as `python3`.
- `assets/` and `scripts/` are local/ignored work areas; do not assume files there are part of the committed package.
